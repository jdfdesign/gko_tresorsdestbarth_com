(function($) {
	...
	aux = {
		...
		setup: function($wrapper, $items, opts) {
			...
			var nav_container = $parent.parent().find('.tj_nav_container:first').html("");
			...
			seqfade: {
				...
				pagination: function($wrapper, dir, opts, page) {
					...
					if (page != undefined && config.currentRow == page) {
						...
						$wrapper.data('anim', false);
						return false;
					} else if ((dir === 1 && config.currentRow + opts.rows > config.totalRows) || (dir === -1 && config.currentRow - opts.rows <= 0)) {
						$wrapper.data('anim', false);
						return false;
					}
				...
				if (this.length) {
					...
					var settings = {
						...
						type: {
							...
							speed: 500,
							// for fade, seqfade, updown, sequpdown, showhide, disperse, rows
							easing: 'jswing',
							// for fade, seqfade, updown, sequpdown, showhide, disperse, rows	
							factor: 50,
							// for seqfade, sequpdown, rows
